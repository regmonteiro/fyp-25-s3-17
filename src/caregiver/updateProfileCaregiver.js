...
import "./updateprofilecaregiver.css";
import { useNavigate } from "react-router-dom";
import Footer from "../footer";

export default function UpdateProfileCaregiver() {
  ...
  const handleSave = async () => {
    ...
    // Include elderlyIds for caregivers
    if (profile.userType === "caregiver") {
      updateData.elderlyIds = controller.getAssignedElderlyIds();
    }

    const success = await controller.updateProfile(updateData);
  ...
  // Get assigned elderly IDs for display
  const getAssignedElderlyIds = () => {
    if (!controller) return [];
    return controller.getAssignedElderlyIds();
                  ...
                  <label className="full-width">
                    <span>
                      ...
                      <span style={{ 
                        ...
                        ({getAssignedElderlyIds().length} assigned)
                      </span>
                    </span>
                    <div style={{ 
                      ...
                      {getAssignedElderlyIds().length > 0 ? (
                        getAssignedElderlyIds().map((id, index) => (
                          <div key={index} style={{ 
                            display: "flex", 
                            ...
                            <span style={{ fontSize: "0.9rem" }}>{id} </span>
                          </div>
                        ))
                      ) : (
                        <div style={{ color: "#999", textAlign: "center" }}>No elderly assigned</div>
                      )}
                    </div>
                  </label>
