...
const DashboardPage = () => {
  ...
  const toggleUserStatus = async (userId, email, currentStatus) => {
    ...
    if (!window.confirm(`Are you sure you want to ${action} the account for ${email}?`)) return;
        ...
        {loadingUsers ? (
          ...
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {['First Name', 'Last Name', 'Email', 'User Type', 'Phone', 'DOB', 'Created At', 'Last Login', 'Status', 'Actions'].map(header => (
              ...
              <tbody>
                {users.map(user => (
                  <tr key={user.id} style={styles.tr}>
                    ...
                    <td style={{ ...styles.td, color: '#005f73' }}>{user.email}</td>
                    <td style={styles.td}>
                    ...
                    <td style={styles.td}>{user.lastLoginDate ? new Date(user.lastLoginDate).toLocaleString() : ''}</td>
                    <td style={{ ...styles.td, fontWeight: '700', color: user.status === 'Deactivated' ? '#d9534f' : '#3c763d' }}>
                      {user.status}
                    </td>
                    <td style={styles.td}>
                      <button
                        ...
                        style={{
                          ...styles.actionButton,
                          backgroundColor: user.status === 'Active' ? '#d9534f' : '#5cb85c', // red for deactivate, green for activate
                        }}
