...
export const ShareExperienceController = {
  ...
  // Get experiences by userKey (optional) - ENHANCED FIREBASE VERIFICATION
  getUserExperiences: (userKey, callback) => {
    const sharedRef = ref(database, "SharedExperiences");
    const accountsRef = ref(database, "Account");
    
    console.log('🔍 Starting to fetch experiences from Firebase...');
    
    // First get all accounts to verify users exist
    onValue(accountsRef, (accountsSnapshot) => {
      const allAccounts = accountsSnapshot.val() || {};
      console.log('📋 Loaded accounts from Firebase:', Object.keys(allAccounts).length);
      ...
      onValue(sharedRef, (experiencesSnapshot) => {
        ...
        // Filter out experiences with anonymous or invalid users WITH FIREBASE VERIFICATION
        const validExperiences = [];
        const invalidExperiences = [];
        
        // Process each experience to check user validity
        const checkExperiences = async () => {
          for (const exp of experiences) {
            // Skip if experience is missing required fields
            if (!exp || !exp.user || !exp.title || !exp.description) {
              console.log('❌ Filtering out invalid experience (missing fields):', exp?.id);
              invalidExperiences.push({...exp, reason: 'Missing required fields'});
              continue;
            }
            
            // Skip if user is "anonymous" or empty
            if (exp.user === "anonymous" || exp.user.trim() === "") {
              console.log('❌ Filtering out anonymous experience:', exp?.id);
              invalidExperiences.push({...exp, reason: 'Anonymous user'});
              continue;
            }
            
            // Skip if title or description is empty
            if (!exp.title.trim() || !exp.description.trim()) {
              console.log('❌ Filtering out empty content experience:', exp?.id);
              invalidExperiences.push({...exp, reason: 'Empty content'});
              continue;
            }
            
            // Check if user exists in Firebase accounts
            const normalizedUser = normalizeEmail(exp.user);
            const userExistsInAccounts = allAccounts[normalizedUser] !== undefined;
            
            if (!userExistsInAccounts) {
              console.log(`❌ Filtering out experience - user not in accounts: ${exp.user}`, exp?.id);
              invalidExperiences.push({...exp, reason: 'User not found in accounts'});
              continue;
            }
            
            // Additional verification: Check if user data is valid in Firebase
            const userAccount = allAccounts[normalizedUser];
            if (!userAccount || !userAccount.email) {
              console.log(`❌ Filtering out experience - invalid user account: ${exp.user}`, exp?.id);
              invalidExperiences.push({...exp, reason: 'Invalid user account'});
              continue;
            }
            
            console.log('✅ Keeping valid experience:', exp.title, 'by', exp.user);
          ...
          console.log('📊 Final results:', {
            ...
            invalid: invalidExperiences.length,
            filtered: filteredExperiences.length,
            invalidReasons: invalidExperiences.reduce((acc, exp) => {
              acc[exp.reason] = (acc[exp.reason] || 0) + 1;
              return acc;
            }, {})
          });
