...
import '../../elderly/boundary/post.dart';
...
class CommunityController extends ChangeNotifier {
  ...
  Future<void> addComment({
    ...
    final comment = Comment(
      userId: uid,
      displayName: currentUserProfile.safeDisplayName,
      text: commentText,
  ...
  Future<List<UserProfile>> searchUsers(String text) async {
  final usersRef = FirebaseFirestore.instance
      .collection('users')
      .withConverter<UserProfile>(
        fromFirestore: (snap, _) => UserProfile.fromDocumentSnapshot(snap),
        toFirestore: (u, _) => u.toFirestoreMap(),
      );

  // Example: prefix search on displayName (adjust to your fields/indexes)
  final qs = await usersRef
      .where('displayName', isGreaterThanOrEqualTo: text)
      .where('displayName', isLessThan: '$text\uf8ff')
      .limit(25)
      .get();

  // docs.map<UserProfile>(...).toList() returns List<UserProfile>
  return qs.docs.map<UserProfile>((d) => d.data()).toList();
}
