...
/// Works with Firestore under `users/{uid}/cards`.
class PaymentController {
  ...
  Future<void> addOrUpdateCard(Map<String, dynamic> payload) async {
    await _db.collection('users').doc(_uid).collection('cards').add(payload);
  }

  /// Delete a card by id
  Future<void> deleteCard(String cardId) async {
    await _db.collection('users').doc(_uid).collection('cards').doc(cardId).delete();
  }
